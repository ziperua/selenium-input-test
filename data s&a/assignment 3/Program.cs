...
public class ProductManager
{
    ...
    public void Sort(string field, bool ascending)
    {
        for (int i = 0; i < products.Count - 1; i++)
        {
            ...
            {
                if (NeedToSwap(products[j], products[j + 1], field, ascending))
                {
                    var temp = products[j];
    ...
    private bool NeedToSwap(Product a, Product b, string field, bool ascending)
    {
        int cmp = 0;
        ...
        {
            case "type":
                cmp = string.Compare(a.Type, b.Type, StringComparison.OrdinalIgnoreCase);
                break;
            case "color":
                cmp = string.Compare(a.Color, b.Color, StringComparison.OrdinalIgnoreCase);
                break;
            case "price":
                cmp = a.CalculateFinalPrice().CompareTo(b.CalculateFinalPrice());
                break;
        }
    ...
    static void Main()
    {
        ...
        while (true)
        {
            ...
            switch (choice)
            {
                ...
                case "8":
                    Console.Write("sort by (type/color/price): ");
                    string field = Console.ReadLine();
                    Console.Write("sort direction (asc/desc): ");
                    string dir = Console.ReadLine();
                    bool ascending = dir.ToLower() == "asc";
